...
class TestOT(object):
    ...
    def update_memory(self):
        ...
        for traj in trajs:
            ...
            for l in range(self.traj_length):
                backward_bound[(l + 1):, l] = self.gamma ** (-l - 1) * (
                        approximate_qs[:len(traj) - l - 1] - backward_return[l:, l])

            for i in range(len(traj)):
                back_len = min(self.traj_length, i + 1)
                forward_len = min(self.traj_length, len(traj) - i)
                self.upper_bound[traj] = min(backward_bound[i, :back_len])
                self.lower_bound[traj] = min(forward_bound[i, :forward_len])
